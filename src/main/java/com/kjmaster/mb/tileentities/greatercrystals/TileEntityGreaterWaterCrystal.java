...
public class TileEntityGreaterWaterCrystal extends TileEntity implements ITickable {
    ...
    public TileEntityGreaterWaterCrystal() {
        this.connections = 0;
    ...
    public void update() {
        if(this.world != null) {
            TileEntityGreaterWaterCrystal te = (TileEntityGreaterWaterCrystal) world.getTileEntity(pos);
                ...
                if(te.getConnections() > 0) {
                    ...
                    for(int i = 0; i < posToSendManaTo.size(); i++) {
                        ...
                        if(entity instanceof TileEntityWaterCrystal) {
                            ...
                            if(waterCrystal.getCanRecieve() && this.storage.canExtract() && !waterCrystal.storage.isFull() && this.storage.getManaStored() >= (MANA_USE / posToSendManaTo.size())) {
                                waterCrystal.receiveMana(MANA_USE / posToSendManaTo.size());
